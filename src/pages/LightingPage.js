...
const LightingPage = () => {
  ...
  const filteredLightings = allProducts.filter(
    (product) =>
      product.category === "Lightings" &&
  ...
  return (
    <Container className="mt-4">
      <h2 className="text-center mb-4">Lightings</h2>
      <Form.Control
        type="text"
        placeholder="Search for lightings..."
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
        className="mb-4"
      />
      <Row>
        {filteredLightings.map((product) => (
