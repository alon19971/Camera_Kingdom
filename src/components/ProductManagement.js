...
const ProductManagement = () => {
  ...
  // Fetch products from Firebase on initial render
  ...
  const handleAddProduct = async () => {
    try {
      ...
      const addedProduct = { id: docRef.id, ...productToAdd };
      setProducts([...products, addedProduct]); // Update local state with the new product
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      setProducts(products.map((product) =>
        product.id === currentProductId ? { ...product, ...updatedProductData } : product
      )); // Update the local state to reflect the edited product immediately
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      ...
      setProducts(products.filter((product) => product.id !== productId)); // Update local state to remove the deleted product
